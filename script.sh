...
if [ -z "$1" ]; then
  echo "Usage: $0 --dump | --dumpfull | --trace [-r]"
  exit 1
fi
...
for arg in "$@"; do
  case "$arg" in
    ...
    --trace)
      ...
      ;;
    *)
      echo "Invalid argument: $arg. Use --dump, --dumpfull, or --trace [-r]"
      exit 1
      ;;
  esac
done
...
case "$action" in
  dump)
    echo "Collecting $dump_type dump for PID: $pid"

    dump_file="core_${COMPUTERNAME}_${timestamp}"
    /tools/dotnet-dump collect -p "$pid" -t "$dump_type" -o "$dump_file"
    ...
    if [ ! -f "$dump_file" ]; then
      ...
      if [ -n "$blob_sas" ]; then
        if upload_file "$dump_file" "$blob_sas"; then
          cleanup_success=1
        fi
      else
  ...
  trace)
    ...
    if [ ! -f "$trace_file" ]; then
      ...
      if [ ! -f "${trace_file}.gz" ]; then
        ...
        if [ -n "$blob_sas" ]; then
          if upload_file "${trace_file}.gz" "$blob_sas"; then
            cleanup_success=1
          fi
        else
          echo "No valid Azure Blob SAS URL found. Skipping upload."
  ...
  *)
    echo "Invalid argument. Use --dump, --dumpfull, or --trace"
    exit 1
    ;;
esac
...
if [ "$cleanup_success" -eq 1 ]; then
    ...
    echo "Upload failed. Keeping dump and trace files for further inspection."
fi
